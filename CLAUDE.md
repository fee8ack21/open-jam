...
| workspace-web | 創作者後台（開店、商品 / 訂單 / 公告管理、上架、收款設定（Stripe Connect）、設定、購買紀錄與下載、收藏清單）+ 管理員後台（會員 / 商店 / 商品 / 訂單 / 金流付款 / 商品分類 / 條款 / 常見問題 / 資源用量 / 稽核） | OIDC（oidc-client-ts） |
...
**Events**（`Shared/Events/`）— `EmailRequestedEvent`、`AuditLogRequestedEvent`、`FileReadyEvent`、`PaymentSucceededEvent`、`OrderCompletedEvent`（fat event，含買家 `BuyerUserId` / `BuyerEmail` 與商品明細）、`CatalogPublishedEvent`、`CatalogVersionReleasedEvent`（創作者對已上架商品版本按「通知買家」）、`StoreFollowerChangedEvent`、`StoreProvisionedEvent`、`UserRegisteredEvent`：各服務在業務 transaction 內寫入 Outbox，由 `OutboxRelayService` 排程搬入 RabbitMQ。
...
- **Cloudflare Turnstile 人機驗證**（`TurnstileService` + `Views/Shared/_Turnstile.cshtml`）— 登入 / 註冊 / 忘記密碼 / 重置密碼四個表單 POST 驗證 `cf-turnstile-response` token（siteverify）。設定於 `Turnstile` 區段（`SiteKey` / `SecretKey`），任一金鑰留空即整組停用（widget 不渲染、後端不驗）；siteverify 連線失敗 fail-open 放行（鎖定與 rate limit 為底線）。本機預設官方測試金鑰（永遠通過），正式 Secret Key 走機密 overlay `auth.turnstile.secretKey`
...
消費 `AuditLogRequestedEvent` 寫入 `audit_log` 表，並提供分頁查詢 REST API（`GET /v1/audit-logs`，Admin，`/swagger`）。
...
- **計數與策展**：`SalesCount`（`OrderCompletedConsumer` 消費 `OrderCompletedEvent` 原子累加，`ProcessedEvent` 去重）、`ViewCount`（`POST /v1/catalogs/{id}/view`，前端以 localStorage 時間窗去重）、`RatingAverage` / `RatingCount`（隨評論維護）、`IsFeatured`（Admin `POST /{id}/feature` / `/unfeature`，首頁精選 × 熱門混合策展）、**店長精選** `IsStoreFeatured`（Owner `POST /{id}/store-feature` / `/store-unfeature`，`PUT /v1/catalogs/store-featured/order` 全量覆蓋重排 `StoreFeaturedSortOrder`）。列表 API 支援排序與價格篩選。
- **評論**：限已購買者，每人每商品一則（`PUT /v1/catalogs/{id}/reviews/mine` upsert / `DELETE` 撤下）。登入買家由 `OrderServiceClient` 轉發 Bearer token 至 OrderService `GET /v1/orders/purchased/{catalogId}` 驗證；**未註冊訪客憑 `?orderId=`**（已完成且含此商品的訂單 ID）匿名驗證，以下單信箱識別（`CatalogReview` 雙身分 `UserId` / `Email`，partial unique index 防重）。
- **買家下載**：`GET /v1/catalogs/{id}/versions/{versionId}/downloads` 回版本可下載檔清單含短效下載 URL。登入買家以購買紀錄授權（同上 `OrderServiceClient`）；訪客憑 `?orderId=`（不可猜測的訂單 ID 作為下載憑證，隨訂單完成信寄出）由 `OrderServiceClient` 匿名查 OrderService 驗證訂單已完成且包含該商品。授權為**商品層級**（任一版本皆可下載），買家永久享有版本更新；creator-web 訂單頁優先取商品目前版本的檔案。
- **新版本通知買家**：`POST /v1/catalogs/{id}/versions/{versionId}/notify-buyers`（Owner）——商品須 `Published` 且版本至少一個可下載檔，每版本至多一次（`CatalogVersion.BuyerNotifiedAt` 防重，重複回 409），經 Outbox 發 `CatalogVersionReleasedEvent`（fat event，含商品 / 版本 / 更新說明）由 NotificationService fan-out 至既有買家。
...
- **建立 Checkout Session**：`POST /v1/payments/checkout-session` 掛 `"InternalService"` policy，僅限 OrderService 以 service token 呼叫（買家不直接接觸本服務）。以 `OrderId` 建 Stripe Checkout Session（`payment_id` / `order_id` 放 Session metadata）；同訂單已有未過期 `Pending` 付款時直接重用既有 Session（含並發 unique violation 時重用勝者），避免重複建立。`POST /v1/payments/expire-by-order/{orderId}`（`InternalService`）供 OrderService 取消訂單前先作廢 Stripe Session（expire-first：已付款的 Session 無法作廢即拒絕取消，關閉取消後仍可付款的競態）。`GET /v1/payments`（Admin）全平台付款分頁列表、`GET /v1/payments/{id}`（Admin）查詢單筆。
...
- **Webhook 兩段式處理**：`POST /v1/webhook/stripe` 驗簽後僅將原始事件落地 `ProviderEvent`（以 Stripe event id 去重）即回 200，避免處理中掛掉遺失 webhook；`StripeWebhookProcessorService`（`IHostedService`）排程處理未完成事件——`checkout.session.completed` / `async_payment_succeeded` → `Succeeded` 並經 Outbox 發 `PaymentSucceededEvent`；`async_payment_failed` → `Failed`；`checkout.session.expired` → 僅 `Pending` 轉 `Expired`（`payment_intent.payment_failed` 刻意不處理：Checkout 內刷卡被拒可原地重試，且事件對不上本地付款）。
...
- **賣方歸屬**：`Order.StoreId`（索引化）標記訂單所屬商店，一張訂單對應單一商店，於 `CreateOrderRequest` 帶入。三種列表視角：買家 `GET /v1/orders/mine`（以登入 `sub` 限縮）、賣家 `GET /v1/orders/store/{storeId}`（`ListByStoreAsync` 先以 `StoreServiceClient` 驗證 Owner 再限縮）、Admin `GET /v1/orders`（`ListOrdersRequest.StoreId` / 買家 / 狀態任意過濾）。列表另支援 `Search` 關鍵字（買家 Email / 訂單編號 ILike 部分比對）。
...
- **購買驗證與取消**：`GET /v1/orders/purchased/{catalogId}` 查登入使用者是否已有該商品的完成訂單（供 CatalogService 評論 / 買家下載授權）；`GET /v1/orders/by-number/{orderNumber}` 憑訂單編號查單筆（guest 兜底）；`POST /v1/orders/{id}/cancel` 取消未付款訂單（具名買家僅本人，匿名訂單憑訂單 ID）——取消前先呼叫 PaymentService `expire-by-order` 作廢 Session（expire-first，已付款則 409 拒絕取消）。
...
- **通知類型**：`catalog.published`（消費 `CatalogPublishedEvent`，僅 `IsFirstPublish` 建任務）、`store.announcement`（Owner 經 `POST /v1/notification-requests` 建立，可帶 `ScheduledAt` 預定發送，僅 `Pending` 可取消）、`catalog.version_released`（消費 `CatalogVersionReleasedEvent`，通知**既有買家**已購商品發布新版本）。
- **fan-out 對象**：追蹤者通知走本地 `store_follower_ref` 參照表（微服務 Ref Table），由 `StoreFollowerChangedEvent` 同步；`catalog.version_released` 走 `catalog_buyer_ref` 買家參照表（每商品每信箱一筆，`OrderId` 留最新完成訂單作為信中下載頁憑證），由 `OrderCompletedEvent` 同步；兩表皆由 `UserRegisteredEvent` 回填訪客 `UserId`（信箱一律小寫）。已關聯帳號者建 in-app `Notification`（`(RequestId, RecipientEmail)` 唯一）；全部收件者經 Outbox 發 `EmailRequestedEvent`（模板 `notification.catalog_published` / `notification.store_announcement` / `notification.catalog_version_released`，版本通知信的 `download_url` 以 `Notification:OrderUrlPattern`（`{storeSlug}` / `{orderId}` 佔位符）逐收件者組出）由 EmailService 寄信。訪客只收 Email。
...
一次性 seed 工具，依序執行 `HydraClientSeeder`（註冊 Hydra OIDC client：Web 與 Service）、`EmailTemplateSeeder`（寫入郵件模板）、`UserSeeder`（建平台管理員，另可選 seed 假帳號）、`LegalDocumentSeeder`（seed 服務條款 / 隱私權政策初始啟用版本至 **ContentService** DB，該類型已有紀錄即略過）、`FaqSeeder`（seed 常見問題主題分類與初始問答至 ContentService DB，取自 portal-web 原前端 FAQ；分類以 slug 冪等 upsert，問答於 FAQ 表已有資料時略過）、`StoreSeeder`（可選 seed 假店家）、`CatalogCategorySeeder`（寫入平台分類）、`StoreFollowerRefSeeder`（回填 NotificationService 追蹤者參照表，重跑冪等）、`CatalogBuyerRefSeeder`（自 OrderService 完成訂單回填 NotificationService 買家參照表，重跑冪等）、`StorefrontRedirectSeeder`（回填既有店家子網域的 OIDC redirect URI 至 Hydra web client，重跑冪等；新店家由 Auth 消費 `StoreProvisionedEvent` 即時註冊）後結束。

掛載 7 個 DbContext：`AuthConnection` / `EmailConnection` / `CatalogConnection` / `StoreConnection` / `NotificationConnection` / `OrderConnection` / `ContentConnection`（具名連線字串，非共用 `DefaultConnection`）。設定 key：`AdminUser:Email` / `:Password`（皆需有值才 seed 管理員）、`MockUsers:Enabled` / `MockStores:Enabled`（正式環境須為 `false`）、`HydraClients:Web` / `:Service`。Helm 由 `templates/bootstrap/job.yaml` 以環境變數注入上述設定。
...
// NotificationService 額外需要（Owner 驗證 + dispatch 時查商店公開資訊 + 信件商品連結）
{
  ...
  "Notification": {
    ...
    "OrderUrlPattern": "https://{storeSlug}.openjam.co/orders/{orderId}",       // 版本更新通知信的買家下載頁連結

...
- **法律文件（`legal_document`）**：`Id`、`Type`（`TermsOfService` / `PrivacyPolicy`）、`Version`（同類型內遞增且唯一）、`Title`、`Content`、`Status`（`Draft` / `Active` / `Inactive`）、`ActivatedAt`（最近一次啟用；null 表從未啟用）。
  - **每次修訂為一筆新紀錄**，版本序號由伺服器依同類型現有最大版本 +1 產生；**永不刪除**（無 DELETE API），供 Auth 同意紀錄與歷史比對。
...
- **僅 `Draft` 可編輯**：對 `Active` / `Inactive` 版本呼叫 PUT 回 409——已啟用過的版本為歷史紀錄，內容凍結。
...
所有寫入操作經 Outbox 發 `AuditLogRequestedEvent`：`legal.create` / `legal.update` / `legal.activate` / `legal.deactivate`、`faq.category.create` / `.update` / `.delete`、`faq.create` / `faq.update` / `faq.delete`。
...
- 資料庫 `open_jam_content`（snake_case、`BaseDbContext` audit 慣例；法律文件與 FAQ 皆不使用軟刪除——前者永不刪除、後者為硬刪除）。

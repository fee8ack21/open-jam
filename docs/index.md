...
- **金流**：Stripe Checkout 串接，支援未登入憑信箱購買，付款成功後完成訂單並開放下載。
- **訂閱與追蹤**：憑信箱追蹤創作者，新品上架與商店公告以 Email + 站內通知送達。
- **社群回饋**：已購買者評論評分、收藏、瀏覽 / 銷量計數與精選策展。
- **帳號系統**：完整註冊／登入／忘記密碼／鎖定流程，OIDC（Hydra）。
- **資源配額**：限制單檔、單一商品與帳號總儲存空間及上架商品數，避免單一用戶吃光雲端空間。
- **內容治理**：不宜內容檢舉。
...
- **後端**：.NET (C#) 微服務，軟體三層 + Service DI。
- **前端**：Vue 3 SPA（TypeScript、vue-i18n）。
- **資料／基礎建設**：Cloud SQL、本地檔案系統／Google Cloud Storage、Redis、Message Queue（Outbox pattern）。
- **部署**：GKE、GitHub Actions + Helm Umbrella Chart、Cloudflare DNS。
- **可觀測性**：Loki + Promtail（Application Log）、Audit Log。

## 規劃筆記

- [[Auth]] — 認證授權（註冊／登入／忘密／OIDC／資安）
- [[Store]] — 商店服務（開店申請、商店資料、追蹤者）
- [[Catalog]] — 商品與商店規格
- [[Order]] — 訂單與金流（Stripe）
- [[Storage]] — 檔案儲存、HLS、安全
- [[Notification]] — 追蹤者通知（上架／公告，Email + in-app）
- [[Email]] — 信件服務與重試
- [[Quota]] — 資源配額計量與統計

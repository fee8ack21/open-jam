...
- 各 SPA 為獨立 OIDC client，皆對 Hydra 走授權碼流程（PKCE）。
- 使用者於 `auth.openjam.co` 登入後由 Hydra 建立 session；其他 SPA 以 silent auth（`prompt=none`）免重登取得 token。
...
- Auth 相關的預建資料（Hydra Client / 各 SPA / API 的 OIDC client、RBAC 角色與權限、初始 Admin 帳號）由 [[Bootstrap]] 統一協調與紀錄。

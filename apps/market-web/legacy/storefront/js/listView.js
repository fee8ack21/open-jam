/* ============================================================
   June — Product list view
   sort · tag filter · price range · favorites · card grid
   ============================================================ */
(function () {
  window.ListView = {
    name: 'ListView',
    components: { ProductCard: window.ProductCard, JIcon: window.JIcon },
    ...
    computed: {
      ...
      cats() { return window.JUNE_CATEGORIES; },
      results() { return this.store.getters.filtered; },
      total() { return window.JUNE_PRODUCTS.length; },
      availableTags() {
        if (this.s.category !== 'all') return window.JUNE_TAGS[this.s.category];
        return [...new Set(Object.values(window.JUNE_TAGS).flat())];
      },
      filterCount() { return this.store.getters.activeFilterCount; },
      priceRange: {
        get() { return this.s.priceRange; },
        set(v) { this.s.priceRange = v; },
      },
    },
    methods: {
          ...
          <div>
            <div class="result-bar">
              ...
              <div style="display:flex; align-items:center; gap:10px;">
                ...
                <n-select v-model:value="s.sort" :options="sortOptions" size="medium" style="width:172px" to=".june-root" />
              </div>
            </div>

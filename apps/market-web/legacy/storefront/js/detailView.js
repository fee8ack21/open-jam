...
(function () {
  ...
  window.DetailView = {
    ...
    computed: {
      p() { return this.store.getters.current; },
      fav() { return this.store.getters.isFav(this.p.id); },
      inCart() { return this.store.getters.inCart(this.p.id); },
      initials() { return this.p.creator.split(' ').map(s => s[0]).slice(0, 2).join(''); },
      catLabel() { return (window.JUNE_CATEGORIES.find(c => c.id === this.p.cat) || {}).label; },
      totalFiles() { return this.p.files.reduce((n, f) => n + f.count, 0); },
    },
    watch: {
      'store.state.currentId'() { this.active = 0; },
    },
    methods: {
      fileColor(t) { return FILE_COLORS[t] || FILE_COLORS.DEFAULT; },
      addCart() { this.store.addToCart(this.p.id); },
      buyNow() { if (!this.inCart) this.store.addToCart(this.p.id); this.store.goCheckout(); },
      goCart() { this.store.goCheckout(); },
    },
    template: `
      <div class="page page-pad" data-screen-label="產品詳細頁" v-if="p">
        <div class="breadcrumb">
          <a @click="store.goList()">探索</a>
          <j-icon name="chevron" :size="14" />
          <a @click="store.goList()">{{ catLabel }}</a>
          <j-icon name="chevron" :size="14" />
          ...
          <div>
            ...
            <div style="margin-top:40px">
              ...
              <div class="preview-locked">
                <div class="lock-veil">
                  <div style="width:46px;height:46px;border-radius:50%;display:grid;place-items:center;background:var(--surface);box-shadow:var(--shadow);color:var(--june-primary)">

...
public class AuditLogPublisher(CatalogDbContext db, IHttpContextAccessor httpContextAccessor)
{
    /// <summary>在目前 DbContext 追蹤範圍內加入一筆審計日誌 Outbox 訊息（需由呼叫端 SaveChanges）。</summary>
    /// <param name="who">執行操作的使用者 ID。</param>
    /// <param name="action">操作代碼，例如 <c>catalog.publish</c>。</param>
    /// <param name="target">目標資源類型，例如 <c>Catalog</c>。</param>

...
| `storage.gcs.gcpServiceAccount` | `""` | GKE Workload Identity 綁定的 GSA email（`Provider: Gcs` 時必填）；chart 建立專屬 KSA 並加 `iam.gke.io/gcp-service-account` annotation，GSA 端 IAM 綁定須先以 gcloud 完成（見 `docs/infra.md`） |
